...
public class AI_Minesweeper {
	...
	public static void neighbourClues() {
		for(int i=0; i<length; i++) {
			for(int j=0; j<width; j++) {
				if(cluesBoard[i][j] != '*' && cluesBoard[i][j] != '?'  && (Math.abs(Character.getNumericValue(cluesBoard[i][j]))-neighbourMines(i, j)) == neighbourUnknowns(i, j)) {
				...
				if(cluesBoard[i][j] != '*' && cluesBoard[i][j] != '?'  && Math.abs(Character.getNumericValue(cluesBoard[i][j])) == 0) {
			...
			for(int j=0; j<probClue[0].length; j++) {
				if(cluesBoard[i][j] != '?' && cluesBoard[i][j] != '0' && cluesBoard[i][j] != '*') {
					...
					if (i-1>=0 && j-1>=0 && cluesBoard[i-1][j-1] != '?') {
						leftTop=copy2DArray(probClue[i-1][j-1]);
						int value=Character.getNumericValue(cluesBoard[i-1][j-1]);
						if(cluesBoard[i-1][j-1]=='*')
							value=0;
						leftTop[1][1]=value;
					}
					if (i-1>=0 && j>=0 && cluesBoard[i-1][j] != '?') {
						centreTop=copy2DArray(probClue[i-1][j]);
						int value=Character.getNumericValue(cluesBoard[i-1][j]);
						if(cluesBoard[i-1][j]=='*')
							value=0;
						centreTop[1][1]=value;
					}
					if (i-1>=0 && j+1<width && cluesBoard[i-1][j+1] != '?') {
						rightTop=copy2DArray(probClue[i-1][j+1]);
						int value=Character.getNumericValue(cluesBoard[i-1][j+1]);
						if(cluesBoard[i-1][j+1]=='*')
							value=0;
						rightTop[1][1]=value;
					...
					if (i>=0 && j-1>=0 && cluesBoard[i][j-1] != '?') {
						leftCentre=copy2DArray(probClue[i][j-1]);
						int value=Character.getNumericValue(cluesBoard[i][j-1]);
						if(cluesBoard[i][j-1]=='*')
							value=0;
						leftCentre[1][1]=value;
					}
					centre=copy2DArray(probClue[i][j]);
					centre[1][1]=Character.getNumericValue(cluesBoard[i][j]);
					if (i>=0 && j+1<width && cluesBoard[i][j+1] != '?') {
						rightCentre=copy2DArray(probClue[i][j+1]);
						int value=Character.getNumericValue(cluesBoard[i][j]);
						if(cluesBoard[i][j]=='*')
							value=0;
						rightCentre[1][1]=value;
					}
					if (i+1<length && j-1>=0 && cluesBoard[i+1][j-1] != '?') {
						leftBot=copy2DArray(probClue[i+1][j-1]);
						int value=Character.getNumericValue(cluesBoard[i+1][j-1]);
						if(cluesBoard[i+1][j-1]=='*')
							value=0;
						leftBot[1][1]=value;
					...
					if (i+1<length && j>=0 && cluesBoard[i+1][j] != '?') {
						centreBot=copy2DArray(probClue[i+1][j]);
						int value=Character.getNumericValue(cluesBoard[i+1][j]);
						if(cluesBoard[i+1][j]=='*')
							value=0;
						centreBot[1][1]=value;
					...
					if (i+1<length && j+1<width && cluesBoard[i+1][j+1] != '?') {
						rightBot=copy2DArray(probClue[i+1][j+1]);
						int value=Character.getNumericValue(cluesBoard[i+1][j+1]);
						if(cluesBoard[i+1][j+1]=='*')
							value=0;
						rightBot[1][1]=value;
					...
					if(centre[1][1] == 0) {
						for(int x=0;x<3;x++) {
							for(int y=0;y<3;y++) {
								if(centre[x][y]==-1)	flagMines(i-1+x, j-1+y);
							}
						}
					}
	...
	public static void flagMines(int i, int j) {
		...
		if (i-1>=0 && j-1>=0)	probClue[i-1][j-1][2][2]=0;
		if (i-1>=0 && j>=0)	probClue[i-1][j][2][1]=0;
		if (i-1>=0 && j+1<width)	probClue[i-1][j+1][2][0]=0;
		if (i>=0 && j-1>=0)	probClue[i][j-1][1][2]=0;
		if (i>=0 && j+1<width)	probClue[i][j+1][1][0]=0;
		if (i+1<length && j-1>=0)	probClue[i+1][j-1][0][2]=0;
		if (i+1<length && j>=0)	probClue[i+1][j][0][1]=0;
		if (i+1<length && j+1<width)	probClue[i+1][j+1][0][0]=0;
